...
ZP = {'V': [17.88, 0.01], 'B': [18.98, 0.02], 'U': [19.36, 0.02],
      'UVW1': [18.95, 0.03], 'UVM2': [18.54, 0.03], 'UVW2': [19.11, 0.03]}
Vega = {'V': -0.01, 'B': -0.13, 'U': 1.02, 'UVW1': 1.51,
        'UVM2': 1.69, 'UVW2': 1.73}
...
def load_obsid(_obsid_string):
    ...
    for root, dirs, files in os.walk("."):
        for file in files:
            if (file.startswith('sw' + obsid) and
                    (file.endswith('_sk.img.gz') or file.endswith('_sk.img'))):
...
def interpret_infile(_infile):
    ...
    for i in range(len(_infile)):
        ...
        if file_exist:
            ...
            if (_infile[i].endswith('_sk.img.gz') or
                    _infile[i].endswith('_sk.img')):
...
def check_aspect_correction(_infile):
    ...
    for i in range(len(hdu)):
        ...
        if hdu[i].header['ASPCORR'] != 'DIRECT':
            ...
            print('WARNING - Extension ' + str(i) + ' ' + ext + ' of ' + _infile +
                  ' has not been aspect corrected. You should check the astrometry.')
    hdu.close()
    del hdu
    ...
    for file in _flist:
        ...
        if len(set(framtime)) == 1:
            ...
            print('WARNING - extensions of ' + file +
                  ' have different FRAMTIMEs. Left unmerged.')
            for i in range(len(hdu)):
                if hdu[i].name == 'PRIMARY':
                    continue
                prod_list.append(file + '[' + str(i) + ']')
...
def extract_photometry(_phot_file, _ab, _det_limit, _ap_size, outdir, _templ_file=None):
    ...
    for i, file in enumerate([_templ_file, _phot_file]):
        ...
        S5BCR = ((dd['RAW_STD_RATE'] * dd['COI_STD_FACTOR'] * SC) -
                 (dd['COI_BKG_RATE'] * SC * dd['STD_AREA']))
        if template and i == 1:
            S5BCRe = np.sqrt((S5CRe)**2 + (S5CR * 0.03)**2 +
                             (dd['COI_BKG_RATE_ERR'] * dd['STD_AREA'])**2)
        else:
        ...
        for BCR, BCRe, label in [[S3BCR, S3BCRe, user_ap], [S5BCR, S5BCRe, '5_arcsec']]:
            if i == 0:
                ...
                ax.errorbar(epochs, BCR, yerr=BCRe, marker='o', color=col[label],
                            label=label)
                ...
                ax.fill_between(xx,
                                [BCR_temp[label] + BCRe_temp[label]] * 2,
                                [BCR_temp[label] - BCRe_temp[label]] * 2,
                                color=col[label], lw=2, alpha=0.2)

                ax.set_xlabel('Epoch')

                print('Galaxy count rates in ' + label.split('_')[0] + '" aperture: '
                      '%.3f +- %.3f' % (BCR_temp[label], BCRe_temp[label]))

            else:
                all_point = []
                mjd = mjdref + (dd['TSTART'] + dd['TSTOP']) / 2. / 86400.
                ax.errorbar(mjd, BCR, yerr=BCRe, marker='o', color=col[label],
                            label=label)
                ...
                if label == user_ap:
                    ...
                    _BCARe = BCRe * dd['AP_FACTOR_ERR']  # noqa: F841
                else:
                    BCGAR = BCGR
                    BCGARe = BCGRe
                    BCAR = BCR
                    _BCARe = BCRe  # noqa: F841

                if template:
                    # Not subtracted magnitudes
                    or_mag = (-2.5 * np.log10(BCAR) + ZP[filter][0] -
                              Vega_corr[filter])
                    or_mage = np.sqrt(((2.5 / np.log(10.)) * ((BCRe / BCAR)))**2 +
                                      ZP[filter][1]**2)

                    mag_host = (-2.5 * np.log10(BCR_temp[label]) + ZP[filter][0] -
                                Vega_corr[filter])
                    mag_hoste = np.sqrt(
                        ((2.5 / np.log(10.)) *
                         ((BCRe_temp[label] / BCR_temp[label])))**2 +
                        ZP[filter][1]**2)

                    ax_mag.errorbar(mjd, or_mag, yerr=or_mage, marker='o',
                                    color=col[label], label=label)

                    ax_mag.plot([min(mjd), max(mjd)], [mag_host] * 2,
                                color=col[label], lw=2)
                    ax_mag.fill_between([min(mjd), max(mjd)],
                                        [mag_host + mag_hoste] * 2,
                                        [mag_host - mag_hoste] * 2,
                                        color=col[label], lw=2, alpha=0.2)

                # determine significance/3 sigma upper limit
                BCGARs = BCGAR / BCGARe
                BCGAMl = (-2.5 * np.log10(3. * BCGARe) + ZP[filter][0] -
                          Vega_corr[filter])
                ...
                for j, CR in enumerate(BCGARs):
                    ...
                    if BCGARs[j] >= _det_limit:
                        BCGAM = (-2.5 * np.log10(BCGAR[j]) + ZP[filter][0] -
                                 Vega_corr[filter])
                        BCGAMe = np.sqrt(
                            ((2.5 / np.log(10.)) * ((BCGARe[j] / BCGAR[j])))**2 +
                            ZP[filter][1]**2)
                    ...
                    # non detection
                        ...
                        print('%.2f\t> %.3f (%.2f)' %
                              (mjd[j], BCGAM, np.fabs(BCGARs[j])))

                    mag[label][-1]['mag'] = float(BCGAM)
                    mag[label][-1]['mag_err'] = float(BCGAMe)
                    all_point.append([mjd[j], BCGAM])

                detections = [[ep['mjd'], ep['mag'], ep['mag_err']]
                              for ep in mag[label] if not ep['upper_limit']]
                non_detections = [[ep['mjd'], ep['mag'], ep['mag_err']]
                                  for ep in mag[label] if ep['upper_limit']]

                if len(detections) > 0:
                    xx, yy, ee = zip(*sorted(detections))
                    ax_sub.errorbar(xx, yy, yerr=ee, marker='o',
                                    color=col[label], label=label, ls='')

                if len(non_detections) > 0:
                    xx, yy, ee = zip(*sorted(non_detections))
                    ax_sub.errorbar(xx, yy,
                                    yerr=[np.abs(np.array(ee)), [0] * len(ee)],
                                    uplims=True, marker='o', color=col[label],
                                    label=label, ls='')
        ...
        if i == 1:
            ...
            if template:
                ...
                out_fig = os.path.join(fig_dir,
                                       file.split('/')[-1][:-5] + '_mag.png')
                if os.path.isfile(out_fig):
                    os.remove(out_fig)
