...
from . import help as SH
from . import uvot as up


def run_swift_photom(infile, outdir, sn_reg='sn.reg', bg_reg='snbkg.reg',
    ...
    if os.path.isdir(os.path.join(outdir, 'reduction')):
        shutil.rmtree(os.path.join(outdir, 'reduction'))

    os.mkdir(os.path.join(outdir, 'reduction'))
    ...
    for filter_name in filt_list:
        ...
        filter_dir = os.path.join(outdir, 'reduction', filter_name)
        ...
        prod_file = up.create_product(obj_file_list[filter_name], filter_name,
                                      outdir, no_combine=no_combine)

        print('Running uvotmaghist on the object image.\n')
        phot_file = up.run_uvotmaghist(prod_file, sn_reg, bg_reg, filter_name, outdir=outdir)
        ...
        if template_exists:
            ...
            prod_file = up.create_product(tem_file_list[filter_name], filter_name,
                                          outdir, template=1, no_combine=no_combine)
    ...
    up.output_mags(mag, ap_size, outdir=outdir)
...
def parse_args(args=None):
    ...
    parser.add_argument("-d", "--detection", dest="det_limit", type=float, default=3.,
                        help=SH.det_limit)
    parser.add_argument("-a", "--ab", dest="ab", default=True, action='store_true',
                        help=SH.ab_mag)
    ...
    parser.add_argument("--no_combine", dest="no_combine", default=0, action='store_true',
                        help=SH.no_combine)
    parser.add_argument("--outdir", dest="outdir", default='.', help=SH.outdir)
...
def main(argv=None):
    """
    Command-line entry point for Swift photometry.
    """
