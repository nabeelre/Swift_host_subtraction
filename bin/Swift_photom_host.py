...
if __name__ == "__main__":
    ...
    for filter in filt_list:
        ...
        if filter not in tem_file_list:
            print('No template provided for filter ' + filter +
        ...
        if template_exists:
            ...
            templ_file = up.run_uvotmaghist(prod_file, args.sn_reg, args.bg_reg,
                                            filter)

            mag_filter = up.extract_photometry(phot_file, args.ab, args.det_limit,
                                               ap_size, templ_file)
